...
*/!function(){
    ...
    function i(t){
        return this instanceof i
            ? (this._canvas=t="string"==typeof t?document.getElementById(t):t,this._ctx=t.getContext("2d"),this._width=t.width,this._height=t.height,this._max=1,void this.clear())
            : new i(t)
    }
    i.prototype = {
        defaultRadius: 25,
        ...
        gradient: function(t) {
            ...
            for(var e in i.width = 1, i.height = 256, t) s.addColorStop(e,t[e]);
        ...
        _colorize: function(t,i) {
            for(var a,s=3,e=t.length;s<e;s+=4)(a=4*t[s])&&(t[s-3]=i[a],t[s-2]=i[1+a],t[s-1]=i[2+a])
        }
    },
    window.simpleheat=i
}(),/*
...
*/L.HeatLayer=(L.Layer?L.Layer:L.Class).extend({
    ...
    _updateOptions: function() {
        this._heat.radius(this.options.radius || this._heat.defaultRadius, this.options.blur), this.options.gradient && this._heat.gradient(this.options.gradient)
    },
    _reset: function() {
    ...
    _redraw: function() {
        if(this._map) {
            var t,i,a,s,e,n,h,o,r=[],
                _=this._heat._r,d=this._map.getSize(),
                l=new L.Bounds(L.point([-_,-_]),d.add([_,_])),
                m=_/2,c=[],u=this._map._getMapPanePos(),f=u.x%m,g=u.y%m;
            for(this._max=1,t=0,i=this._latlngs.length;t<i;t++) {
                a = this._map.latLngToContainerPoint(this._latlngs[t]), e = Math.floor((a.x-f)/m)+2, n=Math.floor((a.y-g)/m)+2;
                var p = void 0 !== this._latlngs[t].alt
                    ? this._latlngs[t].alt
                ...
                c[n] = c[n] || [], 
                    (s = c[n][e])
                    ? (s[0] = (s[0]*s[2]+a.x*p)/(s[2]+p), s[1] = (s[1]*s[2]+a.y*p)/(s[2]+p), s[2] += p, s[3] += 1)
                    : (s = c[n][e] = [a.x, a.y, p, 1]).p=a, s[2] > this._max && (this._max = s[2])
            }
            for(this._heat.max(this._max),t=0,i=c.length;t<i;t++) if(c[t]) for(h=0,o=c[t].length;h<o;h++) (s = c[t][h]) && l.contains(s.p) && r.push([Math.round(s[0]), Math.round(s[1]), Math.min(s[2]/s[3], this._max)]);
            this._heat.data(r).draw(this.options.minOpacity), this._frame=null
        }
    },
